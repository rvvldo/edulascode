...
const CentralAdminDashboard = () => {
  ...
  useEffect(() => {
    if (systemData) {
      ...
      if (systemData.maintenance !== undefined)
        setMaintenanceMode(systemData.maintenance);
    }
  }, [systemData]);
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      toast({
        ...
        description: "Perubahan sistem telah diterapkan.",
      });
    } catch (error) {
      toast({
        title: "Gagal",
  ...
  const users = usersData
    ? Object.entries(usersData).map(([id, u]: [string, any]) => ({ id, ...u }))
    : [];
  const reports = reportsData
    ? Object.entries(reportsData).map(([id, r]: [string, any]) => ({
        id,
        ...r,
      }))
    : [];
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Aktifkan mode perbaikan (hanya admin yang bisa akses).
                    </p>
                  </div>
                  <Switch
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-sm text-muted-foreground">
                    Batas maksimal user yang dapat mendaftar/login.
                  </p>
                </div>
                      ...
                      <TableRow key={user.id}>
                        ...
                        <TableCell className="text-right space-x-2 whitespace-nowrap">
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              handleBlockUser(user.id, user.isActive !== false)
                            }
                            title={
                              user.isActive !== false ? "blokir" : "aktifkan"
                            }
                          >
                            <Ban
                              className={`w-4 h-4 ${user.isActive !== false ? "text-orange-500" : "text-green-500"}`}
                            />
                          </Button>
                          <Button
                            variant="ghost"
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-900">
                <div className="text-2xl font-bold">
                  {
                    reports.filter((r) => r.status === "pending" || !r.status)
                      .length
                  }
                </div>
              ...
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-900">
                <div className="text-2xl font-bold">
                  {reports.filter((r) => r.status === "process").length}
                </div>
            ...
            <div className="space-y-4">
              {reports
                ...
                .map((report) => (
                  <div
                    key={report.id}
                    className="bg-card p-6 rounded-xl border border-border/50 shadow-sm relative"
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Pelapor: {report.userEmail}
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                        className={`${getStatusColor(report.status)} uppercase`}
                      >
                    ...
                    <div className="flex gap-2 justify-end">
                      ...
                      <Button
                        size="sm"
                        variant={
                          report.status === "process" ? "default" : "outline"
                        }
                        className={
                        ...
                        onClick={() => handleReportStatus(report.id, "process")}
                      >
                        Proses
                      </Button>
                      <Button
                        size="sm"
                        variant={
                          report.status === "done" ? "default" : "outline"
                        }
                        className={
                          report.status === "done"
                        ...
                        onClick={() => handleReportStatus(report.id, "done")}
                      >
                        Selesai
                      </Button>
                      {report.status === "done" && (
